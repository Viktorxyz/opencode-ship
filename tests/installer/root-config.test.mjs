 ...
 *   - the engineering profile does NOT inject a Plan Mode
 *     permission block; the consumer keeps its built-in Plan
 *     agent configuration.
 */
...
// Wrap the file-level tests in a suite so node:test reports a
// file-level pass on the suite and a subtest-level report for each
// test. Some test environments report a file-level "test failed"
// artifact when subtests use spawnSync or captureStdout helpers,
// so the suite wrapper avoids the spurious file-level failure.
...
suite("root config", { concurrency: false }, () => {
